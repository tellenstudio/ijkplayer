...
# cronet
CRONET_NAME=cronet.77.0.3850.2
CRONET_INCLUDE_DIR=
CRONET_LIB_DIR=
CRONET_LIB_FOLDER=
...
if [ "$FF_ARCH" = "armv7a" ]; then
    ...
    CRONET_LIB_FOLDER=arm64-v8a
    FF_DEP_LIBS="$FF_DEP_LIBS -lc -lm -landroid -llog"
else
...
CRONET_INCLUDE_DIR=$FF_SOURCE/extra/android/cronet/include
CRONET_LIB_DIR=$FF_SOURCE/extra/android/cronet/libs/$CRONET_LIB_FOLDER
...
#--------------------
# with cronet
if [ -f "${CRONET_LIB_DIR}/lib${CRONET_NAME}.so" ]; then
    echo "Cronet detected"
    FF_CFG_FLAGS="$FF_CFG_FLAGS --enable-libcronet"
    FF_CFG_FLAGS="$FF_CFG_FLAGS --extra-libs=-l${CRONET_NAME}"

    FF_CFLAGS="$FF_CFLAGS -I${CRONET_INCLUDE_DIR}"
    FF_DEP_LIBS="$FF_DEP_LIBS -L${CRONET_LIB_DIR} -l${CRONET_NAME}"
else
	echo "Not find cronet lib file path=${CRONET_LIB_DIR}/lib${CRONET_NAME}.so" 
	exit 1
fi
